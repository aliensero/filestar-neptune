...
mod tests {
    ...
    fn test_mbatch_hash5() {
        ...
        let ctx = cl::default_futhark_context().unwrap();
        let mut state =
            if let BatcherState::Arity5(s) = init_hash5(&mut ctx.lock().unwrap(), Strength::Standard).unwrap() {
                s
            } else {
        ...
        let (hashes, _) = mbatch_hash5(&mut ctx.lock().unwrap(), &mut state, preimages.as_slice()).unwrap();
        let gpu_hashes = gpu_hasher.hash(&preimages).unwrap();
        let expected_hashes: Vec<_> = simple_hasher.hash(&preimages).unwrap();
    ...
    fn test_mbatch_hash5s() {
        ...
        let mut state = if let BatcherState::Arity5s(s) =
            init_hash5(&mut ctx.lock().unwrap(), Strength::Strengthened).unwrap()
        {
            s
        } else {
        ...
        let (hashes, _) = mbatch_hash5s(&mut ctx.lock().unwrap(), &mut state, preimages.as_slice()).unwrap();
        let gpu_hashes = gpu_hasher.hash(&preimages).unwrap();
        let expected_hashes: Vec<_> = simple_hasher.hash(&preimages).unwrap();
